...
def _journal_entries_list(request):
    qs = JournalEntry.objects.filter(company_id=request.company_id).prefetch_related("lines", "lines__account").order_by("-entry_date", "-id")
...
def journal_entry_create(request):
    ...
    for i, row in enumerate(lines):
        ...
        if debit_acc and ChartOfAccount.objects.filter(id=debit_acc, company_id=request.company_id).exists():
            JournalEntryLine.objects.create(journal_entry=e, account_id=debit_acc, debit=amount, credit=0, description=row.get("description") or "")
        if credit_acc and ChartOfAccount.objects.filter(id=credit_acc, company_id=request.company_id).exists():
            JournalEntryLine.objects.create(journal_entry=e, account_id=credit_acc, debit=0, credit=amount, description=row.get("description") or "")
    e.refresh_from_db()
...
def journal_entry_detail(request, entry_id: int):
    e = JournalEntry.objects.filter(id=entry_id, company_id=request.company_id).prefetch_related("lines", "lines__account").first()
    if not e:
...
def journal_entry_post(request, entry_id: int):
    ...
    if not e:
        return JsonResponse({"detail": "Journal entry not found"}, status=404)
    if e.is_posted:
        e_out = JournalEntry.objects.filter(id=entry_id, company_id=request.company_id).prefetch_related("lines", "lines__account").first()
        if not e_out:
            return JsonResponse({"detail": "Journal entry not found"}, status=404)
