"""Feed consumption and medicine consumption reports in the Reports hub."""
...
def test_feed_medicine_consumption_report_groups_by_pond(
    ...
    assert totals["total_medicine_amount"] == "0.00"
    assert totals["total_amount"] == "2600.00"
    ...
    assert alpha["subtotal_medicine_amount"] == "0.00"
    assert alpha["subtotal_amount"] == "1800.00"
    ...
    # Same period on the medicine report: only the pond that used medicine appears.
    mr = api_client.get(
        "/api/reports/aquaculture-medicine-consumption/",
        {"start_date": "2026-05-01", "end_date": "2026-05-31"},
        **auth_admin_headers,
    )
    assert mr.status_code == 200, mr.content.decode()
    mdata = json.loads(mr.content.decode())
    assert len(mdata["groups"]) == 1
    assert mdata["groups"][0]["pond_id"] == p1.id
    assert mdata["totals"]["total_medicine_amount"] == "300.00"
    assert mdata["totals"]["total_feed_amount"] == "0.00"
    assert mdata["totals"]["total_amount"] == "300.00"
    assert len(mdata["groups"][0]["daily_medicine"]) == 1
    assert mdata["groups"][0]["daily_medicine"][0]["date"] == "2026-05-06"
...
def test_feed_medicine_consumption_report_item_filters(
    ...
    assert data["totals"]["total_medicine_amount"] == "0.00"
    assert data["totals"]["total_amount"] == "100.00"
