...
def test_pond_close_snapshots_biological_settlement(api_client, company_tenant, auth_admin_headers):
    """Year close records zero settlement when the pond is empty; blocks close while fish remain."""
    ...
    post_aquaculture_fish_sale_bio_relief_journal(
        cid,
        AquacultureFishSale.objects.filter(pond_id=pond.id).first().id,
        date(2026, 11, 15),
    ...
    preview2 = preview_pond_close(company, pond, date(2026, 12, 31), date(2026, 1, 1))
    assert preview2["is_ready"] is True
    ...
    assert close.settlement_weight_kg == Decimal("0.0000")
