          ...
          <div className="p-6">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="min-w-0">
                ...
                <h2 className="mt-1 text-2xl font-semibold text-gray-900 tracking-tight">Goods Receipt (GRN)</h2>
                <p className="mt-2 text-sm text-gray-600 max-w-3xl">
              ...
              <table className="min-w-full divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    ...
                    <th className="px-4 py-3 text-left text-xs font-semibold text-gray-600 uppercase">PO ref</th>
                  ...
                  {isLoading || isFetching ? (
                    ...
                    <tr>
                      <td colSpan={8} className="px-4 py-8 text-center text-sm text-gray-500">
                        ...
                        <Link href="/purchase/orders" className="font-medium text-indigo-600 hover:text-indigo-800">
                          purchase order
                        </Link>
                        .
                      </td>
                    ...
                    grns.map((g) => (
                      <tr key={g.id} className="hover:bg-gray-50">
                        <td className="px-4 py-3 text-sm font-semibold text-gray-900">{g.grn_number}</td>
                        <td className="px-4 py-3 text-sm text-gray-700">
                          {g.receipt_date ? formatDateOnly(g.receipt_date) : '—'}
                        </td>
                        <td className="px-4 py-3 text-sm text-gray-700">{supplierName(g.supplier_id)}</td>
                        <td className="px-4 py-3 text-sm text-gray-700">{warehouseName(g.warehouse_id)}</td>
                        <td className="px-4 py-3 text-sm text-gray-700">
                          {g.ref_po_id ? (
                            <Link
                        ...
                        <td className="px-4 py-3 text-sm text-right text-gray-900">₹{Number(g.total_amount || 0).toFixed(2)}</td>
