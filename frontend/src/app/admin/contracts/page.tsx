        ...
        <div className="app-scroll-pad">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-muted-foreground mt-1">Manage contracts for all companies</p>
            </div>
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-lg shadow app-modal-pad text-center">
              ...
              <p className="text-muted-foreground text-sm">Add companies from the Admin dashboard first.</p>
            </div>
          ) : contracts.length === 0 ? (
            ...
            <div className="space-y-6">
              {companies.map((company) => {
                ...
                return (
                  <div key={company.id} className="bg-white rounded-lg shadow-lg border border-border">
                    <div className="p-6">
                      {/* Company Header */}
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          <div className="flex items-center space-x-3 mb-2">
                            ...
                            {activeContract && (
                              <span className="px-3 py-1 text-xs font-semibold bg-blue-100 text-primary rounded-full">
                                Has Active Contract
                              </span>
                            )}
                          </div>
                          {company.legal_name && (
                            <p className="text-sm text-muted-foreground ml-8">{company.legal_name}</p>
                          )}
                          <div className="mt-2 ml-8 flex items-center space-x-4 text-sm text-muted-foreground">
                      ...
                      <div className="mt-4 border-t border-border pt-4">
                        <div className="flex items-center justify-between mb-3">
                          <h4 className="text-sm font-semibold text-foreground/85">Contracts ({companyContracts.length})</h4>
                        ...
                        {companyContracts.length === 0 ? (
                          <p className="text-sm text-muted-foreground italic">No contracts for this company</p>
                        ) : viewMode === 'card' ? (
                          // Card View
                          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {companyContracts.map((contract) => (
                              <div
                                key={contract.id}
                                className="bg-muted/40 rounded-lg p-4 border border-border hover:bg-muted transition-colors"
                              >
                                <div className="flex items-start justify-between mb-3">
                                  ...
                                  <div className="flex items-center space-x-1 ml-2 flex-shrink-0">
                                    <button
                                      ...
                                      className="p-1.5 text-muted-foreground hover:bg-muted/40 rounded transition-colors border border-transparent hover:border-border"
                                      title="View Contract"
                                    ...
                                    <button
                                      ...
                                      className="p-1.5 text-primary hover:bg-accent rounded transition-colors border border-transparent hover:border-primary/25"
                                    ...
                                    <button
                                      ...
                                      className="p-1.5 text-success hover:bg-green-50 rounded transition-colors border border-transparent hover:border-success/25"
                                      title="Edit Contract"
                                    ...
                                    <button
                                      ...
                                      className="p-1.5 text-destructive hover:bg-destructive/5 rounded transition-colors border border-transparent hover:border-destructive/25"
                                ...
                                <div className="space-y-2 text-sm">
                                  <div className="flex justify-between">
                                    ...
                                    <span className="text-foreground">{formatDateOnly(contract.contract_date)}</span>
                                  </div>
                                  <div className="flex justify-between">
                                    <span className="text-muted-foreground">Expiry:</span>
                                    <span className={`${isExpiringSoon(contract.expiry_date) ? 'text-destructive font-semibold' : 'text-foreground'}`}>
                                  ...
                                  <div className="flex justify-between">
                                    ...
                                    <span className="text-foreground capitalize">{contract.billing_period}</span>
                                  </div>
                          ...
                          <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-border">
                              <thead className="bg-muted/40">
                                <tr>
                                  <th className="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase">Contract #</th>
                                  <th className="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase">Status</th>
                                  <th className="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase">Start Date</th>
                                  <th className="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase">Expiry Date</th>
                                  <th className="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase">Amount</th>
                                  <th className="px-4 py-2 text-left text-xs font-medium text-muted-foreground uppercase">Billing</th>
                              ...
                              <tbody className="bg-white divide-y divide-border">
                                {companyContracts.map((contract) => (
                                  <tr key={contract.id} className="hover:bg-muted/40">
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap">
                                      <span className={`text-sm ${isExpiringSoon(contract.expiry_date) ? 'text-destructive font-semibold' : 'text-foreground'}`}>
                                        {formatDateOnly(contract.expiry_date)}
                                      </span>
                                    </td>
                                    ...
                                    <td className="px-4 py-3 whitespace-nowrap text-sm text-foreground capitalize">
                ...
                <div className="p-6">
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        ...
                        <p className="text-sm text-foreground font-semibold">{viewingContract.contract_number}</p>
                      </div>
                      <div>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-foreground">{formatDateOnly(viewingContract.contract_date)}</p>
                      </div>
                      <div>
                        <label className="text-sm font-medium text-muted-foreground">Expiry Date</label>
                        <p className={`text-sm ${isExpiringSoon(viewingContract.expiry_date) ? 'text-destructive font-semibold' : 'text-foreground'}`}>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-foreground capitalize">{viewingContract.billing_period}</p>
                      </div>
                      {viewingContract.license_type && (
                        <div>
                          <label className="text-sm font-medium text-muted-foreground">License Type</label>
                          <p className="text-sm text-foreground">{viewingContract.license_type}</p>
                        </div>
                      )}
                      {viewingContract.auto_renewal === 'true' && (
                        <div>
                          <label className="text-sm font-medium text-muted-foreground">Auto Renewal</label>
                          <p className="text-sm text-success font-semibold">Enabled</p>
                        </div>
                      )}
                    </div>
                    {viewingContract.broadcast_message && (
                      <div>
                        <label className="text-sm font-medium text-muted-foreground">Broadcast Message</label>
                        <p className="text-sm text-foreground mt-1">{viewingContract.broadcast_message}</p>
                      </div>
                    )}
                    {viewingContract.terms_and_conditions && (
                      <div>
                        <label className="text-sm font-medium text-muted-foreground">Terms and Conditions</label>
                        <p className="text-sm text-foreground mt-1 whitespace-pre-wrap">{viewingContract.terms_and_conditions}</p>
                      </div>
                    )}
                    {viewingContract.notes && (
                      <div>
                        <label className="text-sm font-medium text-muted-foreground">Notes</label>
                        <p className="text-sm text-foreground mt-1 whitespace-pre-wrap">{viewingContract.notes}</p>
                      </div>
                    )}
                  </div>
                  <div className="mt-6 flex justify-end gap-2">
                    <button
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary"
                    >
                      <Printer className="h-4 w-4" />
                      Print
                    </button>
                    <button
                      type="button"
                      onClick={() => setViewingContract(null)}
                      className="px-4 py-2 bg-muted text-foreground/85 rounded-lg hover:bg-muted"
                    >
                      Close
                    </button>
                  </div>
