...
const BILLING_PLANS: BillingPlan[] = [
  ...
  },
  {
    ...
    icon: Building2,
    accent: 'from-slate-600 to-[hsl(var(--hero-from))]',
    borderAccent: 'border-border hover:border-border',
  ...
  },
  {
    ...
    borderAccent: 'border-warning/30 hover:border-amber-300',
              ...
              <div className="flex flex-col sm:items-end gap-3">
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => refreshAll()}
                    className="inline-flex items-center gap-2 rounded-lg bg-white text-foreground px-3 py-2 text-sm font-medium hover:bg-accent shadow-sm"
                  >
          ...
          <div className="max-w-7xl mx-auto space-y-8">
            <section aria-label="Platform metrics">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
                <div className="rounded-2xl border border-border/80 bg-white p-4 sm:p-5 shadow-sm ring-1 ring-slate-900/5">
                  ...
                  <p className="text-2xl font-bold text-foreground mt-2 tabular-nums">{kpis.total}</p>
                  <p className="text-xs text-muted-foreground mt-1">{kpis.active} active · dated subs {kpis.withEnd}</p>
                </div>
                <div className="rounded-2xl border border-warning/30/80 bg-gradient-to-br from-amber-50 to-card p-4 sm:p-5 shadow-sm">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-xs font-semibold uppercase tracking-wide text-warning-foreground/70">Renewal window</p>
                    <CalendarClock className="h-5 w-5 text-amber-600 shrink-0" />
                  </div>
                  <p className="text-2xl font-bold text-warning-foreground mt-2 tabular-nums">{kpis.expiring}</p>
                  <p className="text-xs text-warning-foreground/70 mt-1">Subscription end within 30 days</p>
                </div>
                <div className="rounded-2xl border border-destructive/25/80 bg-gradient-to-br from-red-50/80 to-card p-4 sm:p-5 shadow-sm">
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-xs font-semibold uppercase tracking-wide text-destructive/70">Lapsed periods</p>
                    <TrendingUp className="h-5 w-5 text-red-500 rotate-180 shrink-0" />
                  </div>
                  <p className="text-2xl font-bold text-destructive mt-2 tabular-nums">{kpis.expired}</p>
                  <p className="text-xs text-destructive/70 mt-1">Past payment end date</p>
                </div>
                <div className="rounded-2xl border border-emerald-200/80 bg-gradient-to-br from-emerald-50/90 to-card p-4 sm:p-5 shadow-sm">
                  <div className="flex items-center justify-between gap-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mt-4">
                <div className="rounded-xl border border-border bg-white px-4 py-3 flex items-center justify-between gap-3 shadow-sm">
                  <div>
                    <p className="text-xs font-medium text-muted-foreground">Ledger — marked paid</p>
                    <p className="text-lg font-semibold text-foreground tabular-nums">
                      {formatCurrency(ledgerKpis.paid, 'BDT')}
                    </p>
                  </div>
                  <span className="text-xs text-muted-foreground/70 max-w-[10rem] text-right">
                    Numeric sum of SaaS ledger; confirm currency per tenant.
                  </span>
                </div>
                <div className="rounded-xl border border-border bg-white px-4 py-3 flex items-center justify-between gap-3 shadow-sm">
                  <div>
                    <p className="text-xs font-medium text-muted-foreground">Ledger — open (excl. void)</p>
                    <p className="text-lg font-semibold text-foreground tabular-nums">
                ...
                <div className="rounded-xl border border-border bg-white px-4 py-3 shadow-sm">
                  <p className="text-xs font-medium text-muted-foreground mb-1">Invoice status mix</p>
                  <div className="flex flex-wrap gap-2">
                    {['paid', 'sent', 'overdue', 'draft', 'void'].map((s) => (
                      <span
                        key={s}
                        className="inline-flex items-center gap-1 rounded-full bg-muted text-foreground/85 text-xs px-2 py-0.5 capitalize"
                      >
                        {s}
              ...
              <div className="flex flex-wrap items-end justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1 max-w-3xl">
                    ...
                    Use the pencil to tune <strong className="font-medium text-foreground">reference amounts</strong>{' '}
                    in this browser; trash restores defaults. Canonical catalog is on the server (
                    <code className="text-xs bg-muted px-1 rounded">GET /api/admin/billing-plans/</code>).
                  </p>
                </div>
                {!selectedRow && (
                  <p className="text-xs text-warning-foreground bg-warning/10 border border-warning/30 rounded-lg px-3 py-2">
                    ...
                    >
                      <div className="absolute top-3 right-3 flex items-center gap-0.5">
                        ...
                        <button
                          ...
                          className="p-2 rounded-lg text-muted-foreground hover:bg-destructive/5 hover:text-destructive disabled:opacity-30 disabled:hover:bg-transparent transition-colors"
                        >
                          <Trash2 className="h-4 w-4" />
                        </button>
                      </div>
                      {plan.recommended && (
                        <span className="absolute -top-2.5 left-4 rounded-full bg-primary text-primary-foreground text-[10px] font-bold uppercase tracking-wider px-2 py-0.5">
                          Popular
                        </span>
                      )}
                      <div
                      ...
                      <h3 className="text-base font-bold text-foreground pr-16">{plan.name}</h3>
                      <p className="text-xs text-muted-foreground mt-0.5 mb-3">{displayPlan.tagline}</p>
                      ...
                      <ul className="text-xs text-muted-foreground space-y-1.5 mb-4 min-h-[5.5rem]">
                      ...
                      <button
                        ...
                        className="w-full rounded-xl bg-foreground text-white text-sm font-medium py-2.5 hover:bg-foreground/90 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                      >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
              <div className="lg:col-span-7 space-y-3">
                <div className="rounded-2xl border border-border/80 bg-white shadow-sm overflow-hidden ring-1 ring-slate-900/5">
                  <div className="p-4 sm:p-5 border-b border-border/70 bg-muted/40">
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        ...
                        <div className="relative flex-1 min-w-[12rem] max-w-md">
                          ...
                          <input
                            ...
                            className="w-full pl-9 pr-3 py-2.5 rounded-xl border border-border text-sm bg-white focus:ring-2 focus:ring-ring focus:border-ring"
                          />
                        </div>
                      </div>
                      <div className="flex flex-wrap gap-2">
                  ...
                  {loading ? (
                    <div className="p-16 text-center text-muted-foreground text-sm">Loading companies…</div>
                  ) : (
                    <div className="overflow-x-auto max-h-[min(28rem,50vh)] lg:max-h-[calc(100vh-18rem)] overflow-y-auto">
                      <table className="min-w-full text-sm">
                        <thead className="bg-muted/40 text-left sticky top-0 z-10 shadow-sm">
                          <tr>
                            <th className="px-4 py-3 font-semibold text-foreground/85">Company</th>
                            <th className="px-4 py-3 font-semibold text-foreground/85">Period</th>
                            <th className="px-4 py-3 font-semibold text-foreground/85 text-right">Amount</th>
                            <th className="px-4 py-3 font-semibold text-foreground/85">Status</th>
                            <th className="px-4 py-3 font-semibold text-foreground/85 text-right w-[7rem]">Actions</th>
                          </tr>
                        </thead>
                        <tbody>
                          {filtered.map((c) => {
                            ...
                            return (
                              <tr
                                ...
                                className={`border-t border-border/70 cursor-pointer transition-colors ${
                                  sel ? 'bg-accent/90' : 'hover:bg-muted/50'
                                }`}
                              ...
                              >
                                ...
                                <td className="px-4 py-3 text-muted-foreground">
                                  ...
                                  {c.payment_end_date ? (
                                    <div className="tabular-nums text-foreground">Until {c.payment_end_date.split('T')[0]}</div>
                                  ) : (
                                    <span className="text-muted-foreground/70">No end date</span>
                                  )}
                                </td>
                                ...
                                <td className="px-4 py-3">
                                  {h === 'none' && (
                                    <span className="inline-flex rounded-full bg-muted text-muted-foreground text-xs px-2.5 py-0.5 font-medium">
                                      Not set
                                    </span>
                                  )}
                                  {h === 'active' && (
                                  ...
                                  {h === 'expiring' && (
                                    <span className="inline-flex rounded-full bg-amber-100 text-warning-foreground text-xs px-2.5 py-0.5 font-medium">
                                      Renew soon
                                    </span>
                                  )}
                                  {h === 'expired' && (
                                    <span className="inline-flex rounded-full bg-destructive/10 text-destructive text-xs px-2.5 py-0.5 font-medium">
                                      Expired
                                    </span>
                                  )}
                                </td>
                      ...
                      {filtered.length === 0 && (
                        <p className="p-10 text-center text-muted-foreground text-sm">No tenants match filters.</p>
                      )}
                    </div>
                  )}
                </div>
              ...
              <div className="lg:col-span-5 space-y-4 lg:sticky lg:top-4">
                {!selectedId || !selectedRow ? (
                  <div className="rounded-2xl border border-dashed border-border bg-white/90 p-10 text-center shadow-sm">
                    <FileText className="h-10 w-10 text-muted-foreground/40 mx-auto mb-3" />
                    <p className="text-foreground/85 font-semibold">Select a tenant</p>
                  ...
                  <div className="rounded-2xl border border-border bg-white p-10 text-center text-muted-foreground text-sm shadow-sm">
                    Loading billing data…
                  </div>
                ) : (
                  <>
                    <div className="rounded-2xl border border-border bg-white shadow-sm p-5 ring-1 ring-slate-900/5">
                      ...
                      <p className="text-xs text-muted-foreground mt-1.5">
                        ...
                        <span className="font-medium text-foreground/85">{selectedRow.currency}</span>
                      </p>
                    </div>

                    <div className="rounded-2xl border border-primary/25/80 bg-gradient-to-b from-accent/80 to-card shadow-sm p-5 space-y-4 ring-1 ring-indigo-900/5">
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <p className="text-xs text-foreground/85/75 mt-1 leading-relaxed">
                        ...
                        <Link
                          href="/admin/contracts"
                          className="shrink-0 text-xs font-medium text-primary hover:text-foreground/85 inline-flex items-center gap-0.5"
                        >
                          Open
                      ...
                      {companyContracts.length === 0 ? (
                        <p className="text-sm text-muted-foreground bg-white/80 rounded-xl border border-primary/15 px-3 py-3">
                        ...
                        <ul className="space-y-2 max-h-40 overflow-y-auto">
                          {companyContracts.slice(0, 6).map((ct) => {
                            ...
                            return (
                              <li
                                key={ct.id}
                                className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-primary/15 bg-white px-3 py-2.5 text-xs"
                              >
                                <div>
                                  <span className="font-mono font-semibold text-foreground">{ct.contract_number}</span>
                                  <span className="text-muted-foreground/70 mx-1">·</span>
                                  <span className="capitalize text-muted-foreground">{ct.status}</span>
                                  {ct.expiry_date && (
                                ...
                                <div className="flex items-center gap-1 shrink-0">
                                  {ch === 'expiring' && (
                                    <span className="rounded-full bg-amber-100 text-warning-foreground px-2 py-0.5 font-medium">
                                      Expiring
                                    </span>
                                  )}
                                  {ch === 'expired' && (
                      ...
                      <button
                        ...
                        className="w-full rounded-xl border border-primary/30 bg-white text-foreground/85 text-sm font-medium py-2.5 hover:bg-accent disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                      >
                        Copy terms from active contract into billing profile
                      </button>
                    </div>

                    <div className="rounded-2xl border border-border bg-white shadow-sm p-5 space-y-4 ring-1 ring-slate-900/5">
                      <h4 className="text-sm font-semibold text-foreground flex items-center gap-2">
                      ...
                      <div className="grid gap-3">
                        <label className="block">
                          <span className="text-xs font-semibold text-muted-foreground">Billing plan</span>
                          <select
                            value={billingForm.billing_plan_code}
                            onChange={(e) => setBillingForm({ ...billingForm, billing_plan_code: e.target.value })}
                            className="mt-1.5 w-full rounded-xl border border-border px-3 py-2.5 text-sm bg-white focus:ring-2 focus:ring-ring"
                          >
                        ...
                        <label className="block">
                          <span className="text-xs font-semibold text-muted-foreground">Billing cycle</span>
                          <select
                        ...
                        <label className="block">
                          <span className="text-xs font-semibold text-muted-foreground">Quoted recurring amount</span>
                          <input
                            type="text"
                            inputMode="decimal"
                        ...
                        <button
                          ...
                          className="w-full rounded-xl bg-foreground text-white text-sm font-semibold py-3 hover:bg-foreground/90 disabled:opacity-50 transition-colors"
                        >
                    ...
                    <div className="rounded-2xl border border-primary/25 bg-accent/60 shadow-sm p-5 space-y-3">
                      <h4 className="text-sm font-semibold text-foreground flex items-center gap-2">
                      ...
                      <div className="flex flex-wrap gap-2 items-end">
                        ...
                        <button
                          ...
                          disabled={extending}
                          onClick={applyExtend}
                          className="rounded-xl bg-primary text-primary-foreground text-sm font-semibold px-5 py-2.5 hover:bg-primary/90 disabled:opacity-50 transition-colors"
                        >
                    ...
                    <div className="rounded-2xl border border-border bg-white shadow-sm p-5 space-y-4 ring-1 ring-slate-900/5">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <button
                          ...
                          className="inline-flex items-center gap-1 rounded-lg bg-primary text-primary-foreground text-xs font-semibold px-3 py-2 hover:bg-primary/90"
                        >
                      ...
                      <p className="text-xs text-muted-foreground leading-relaxed">
                        ...
                        <Link href="/admin/subscription-ledger" className="text-primary font-medium hover:underline">
                          Global ledger view
                        </Link>
                        .
                      </p>
                      ...
                      {showInvoiceForm && (
                        <div className="rounded-xl border border-border bg-muted/40 p-4 space-y-3 text-sm">
                          <div className="flex justify-between items-center">
                            ...
                            <button type="button" onClick={() => setShowInvoiceForm(false)} className="text-muted-foreground p-1">
                              <X className="h-4 w-4" />
                            </button>
                          </div>
                          <div className="grid grid-cols-2 gap-2">
                            <select
                              ...
                              className="rounded-lg border border-border px-2 py-2 text-sm col-span-2"
                            >
                      ...
                      <div className="max-h-56 overflow-y-auto border border-border/70 rounded-xl">
                        {!bundle?.invoices?.length ? (
                          <p className="p-6 text-sm text-muted-foreground text-center">No ledger invoices yet.</p>
                        ) : (
                          <table className="min-w-full text-xs">
                            <thead className="bg-muted/40 text-left sticky top-0 z-[1]">
                              <tr>
                                <th className="px-3 py-2 font-semibold text-foreground/85">#</th>
                                <th className="px-3 py-2 font-semibold text-foreground/85 text-right">Amount</th>
                                <th className="px-3 py-2 font-semibold text-foreground/85">Status</th>
                                <th className="px-3 py-2 w-16" />
                              </tr>
                            </thead>
                            <tbody>
                              {bundle.invoices.map((inv) => (
                                <tr key={inv.id} className="border-t border-border/70 hover:bg-muted/40">
                                  <td className="px-3 py-2">
                                    <div className="font-mono text-foreground">{inv.invoice_number}</div>
                                    <div className="text-muted-foreground/70">{inv.invoice_date || '—'}</div>
                                  </td>
                                  ...
                                  <td className="px-3 py-2 text-right whitespace-nowrap">
                                    <div className="inline-flex items-center gap-0.5 justify-end">
                                      ...
                                      <button
                                        ...
                                        className="text-primary hover:bg-accent p-1 rounded"
                                        title="Edit"
                                      >
                                      ...
                                      <button
                                        ...
                                        className="text-destructive hover:bg-destructive/5 p-1 rounded"
                                        title="Delete"
                                      >
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4 bg-black/40 backdrop-blur-[2px]">
          <div className="bg-white rounded-2xl shadow-2xl max-w-lg w-full p-6 space-y-4 ring-1 ring-slate-900/10 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-start gap-3">
              <div>
                <h3 className="text-lg font-semibold text-foreground">Edit reference plan</h3>
              ...
              <button
                ...
                className="text-muted-foreground/70 hover:text-muted-foreground p-1 shrink-0"
              >
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="space-y-3 text-sm">
              ...
              <div className="grid grid-cols-2 gap-2">
                <label className="block">
                  <span className="text-xs font-semibold text-muted-foreground">Monthly (BDT)</span>
                  <input
                    type="number"
                    step="0.01"
                ...
                <label className="block">
                  <span className="text-xs font-semibold text-muted-foreground">Yearly (BDT)</span>
                  <input
                    type="number"
                    step="0.01"
              ...
              <label className="block">
                <span className="text-xs font-semibold text-muted-foreground">Feature bullets (one per line)</span>
                <textarea
                  rows={6}
                  value={planEditForm.featuresText}
                  onChange={(e) => setPlanEditForm({ ...planEditForm, featuresText: e.target.value })}
                  className="mt-1 w-full border border-border rounded-xl px-3 py-2 font-mono text-xs"
                />
              </label>
            </div>
            <div className="flex flex-wrap gap-2 justify-end pt-2">
              <button
                type="button"
                onClick={() => setPlanEditOpen(null)}
                className="px-4 py-2 rounded-xl border border-border text-sm font-medium hover:bg-muted/40"
              >
                Cancel
              </button>
              <button
                ...
                className="px-4 py-2 rounded-xl border border-destructive/25 text-destructive text-sm font-medium hover:bg-destructive/5"
              >
                Reset default
              </button>
              <button
                ...
                className="px-4 py-2 rounded-xl bg-foreground text-white text-sm font-semibold hover:bg-foreground/90"
              >
                Save
              </button>
            </div>
      ...
      {editingInvoice && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/40 backdrop-blur-[2px]">
          <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 space-y-4 ring-1 ring-slate-900/10">
            <div className="flex justify-between items-center">
              <h3 className="text-lg font-semibold text-foreground">Edit invoice</h3>
              <button type="button" onClick={() => setEditingInvoice(null)} className="text-muted-foreground/70 hover:text-muted-foreground p-1">
                <X className="h-5 w-5" />
              </button>
            </div>
            <div className="space-y-3 text-sm">
