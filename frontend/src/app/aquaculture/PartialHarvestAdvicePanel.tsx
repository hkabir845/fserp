    ...
    >
      <div className="flex flex-wrap items-start gap-3">
        <div className="min-w-0 flex-1">
          <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Pond load</p>
          <div className="mt-1 flex flex-wrap items-center gap-2">
            ...
            {density ? (
              ...
              <span className="text-sm text-slate-500">Set water area (decimal) on pond</span>
            )}
          </div>
          {pcs ? (
            <p className="mt-1 text-sm text-slate-600">
              Current size: <span className="font-medium tabular-nums">{pcs}</span>
          ...
          {row.implied_net_fish_count != null && row.implied_net_weight_kg ? (
            <p className="mt-1 text-sm text-slate-600">
              ...
              <span className="font-medium tabular-nums">
                {formatNumber(row.implied_net_fish_count, 0)} fish ·{' '}
            ...
            <p className="mt-1 text-base font-bold tabular-nums text-amber-950">
              ...
              {suggestHeads != null && suggestHeads > 0 ? (
                <span className="ml-1 text-sm font-semibold text-amber-800">
                  (~{formatNumber(suggestHeads, 0)} fish)
                </span>
              ) : null}
            </p>
            {row.partial_harvest_post_load_kg_per_decimal ? (
              <p className="text-xs text-amber-800/90">
                → ~{formatNumber(Number(row.partial_harvest_post_load_kg_per_decimal), 1)} kg/decimal after
              </p>
            ) : null}
          </div>
        ) : null}
      ...
      {applicable ? (
        <p className="mt-2 text-[11px] text-amber-800/80">
          Advisory only — you may harvest more or less based on water quality, market, and growth targets.
        </p>
      ) : null}
    </div>
  )
