...
const selectCls =
  'rounded-lg border border-border bg-white px-2 py-1.5 text-sm shadow-sm focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20'
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <div className="mt-6 rounded-2xl border border-warning/30 bg-warning/10 px-4 py-5 text-sm text-warning-foreground">
          <p className="font-medium">Add at least one pond first</p>
          <Link href="/aquaculture/ponds" className="mt-3 inline-block font-medium text-primary underline">
            Go to Ponds
          </Link>
        </div>
      ) : (
        <div className="mt-6 grid gap-6 lg:grid-cols-12 lg:items-start">
          <div className="space-y-4 lg:col-span-5 xl:col-span-5">
            ...
            <div className="rounded-2xl border border-border/80 bg-white p-3 shadow-sm">
              <h2 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">History filters</h2>
              ...
              <button
                ...
                onClick={() => void loadLedger()}
                disabled={ledgerLoading}
                className="mt-3 flex w-full items-center justify-center gap-2 rounded-lg border border-border bg-muted/40 py-2 text-xs font-semibold text-foreground hover:bg-muted disabled:opacity-50"
              >
                <RefreshCw className={`h-3.5 w-3.5 ${ledgerLoading ? 'animate-spin' : ''}`} aria-hidden />
                Apply date range
              </button>
          ...
          <div className="space-y-4 lg:col-span-7 xl:col-span-7">
            ...
            <p className="rounded-xl border border-teal-100 bg-accent/60 px-3 py-2.5 text-xs leading-relaxed text-teal-950">
            ...
            <div className="overflow-hidden rounded-2xl border border-border/80 bg-white shadow-sm ring-1 ring-border/50">
              <div className="border-b border-border/70 bg-muted/50 px-4 py-3">
                <h2 className="text-sm font-semibold text-foreground">Treatment history</h2>
            ...
            <section className="rounded-2xl border border-border/80 bg-white p-4 shadow-sm">
              ...
              >
                ...
                {showDefaultSku ? (
                  <ChevronUp className="h-4 w-4 text-muted-foreground/70" aria-hidden />
                ) : (
