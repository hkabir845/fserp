  ...
  return (
    <PageLayout>
        ...
        actions={
          undepositedPayments.length > 0 && !showDepositForm ? (
            <button
              ...
              className="inline-flex shrink-0 items-center gap-2 rounded-full bg-amber-400 px-4 py-2 text-sm font-semibold text-foreground hover:bg-amber-300"
            >
      ...
      >
          ...
          {listError && (
            <div className="mb-6 flex items-center gap-2 rounded-lg border border-destructive/25 bg-destructive/5 px-4 py-3 text-destructive">
          ...
          <div className="mb-6 rounded-xl border border-border bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-start">
              ...
              <div className="min-w-0 flex-1 space-y-2 text-sm text-muted-foreground">
                <h2 className="text-base font-semibold text-foreground">How this fits your books</h2>
                <p>
                  When you record a <strong className="font-medium text-foreground">payment received</strong>{' '}
                  without choosing a bank register, the system posts to your{' '}
                  <strong className="font-medium text-foreground">clearing</strong> asset (e.g. cash on hand,
                  undeposited funds, or card clearing — per your chart of accounts). This screen creates a
                  single <strong className="font-medium text-foreground">bank deposit</strong>: debit the
          ...
          <div className="mb-6 grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-xl border border-border bg-white p-5 shadow-sm">
              <div className="flex items-center gap-3">
                <div className="flex h-10 w-10 items-center justify-center rounded-lg bg-muted text-foreground/85">
                  <Building2 className="h-5 w-5" aria-hidden />
                </div>
                <div>
                  <p className="text-xs font-medium uppercase tracking-wide text-muted-foreground">Status</p>
                  {undepositedPayments.length > 0 ? (
                    <p className="text-sm font-medium text-warning-foreground">Deposit queue has open items</p>
                  ) : (
                    <p className="text-sm font-medium text-emerald-800">No undeposited receipts</p>
                  )}
                  <p className="text-sm text-muted-foreground">Reconcile the bank register after the real deposit clears.</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="mb-8 rounded-xl border border-border bg-white shadow-sm">
              <div className="border-b border-border/70 px-5 py-4 sm:px-6">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <h2 className="text-lg font-semibold text-foreground">New bank deposit</h2>
                  <button
                    type="button"
                    onClick={handleCancelDeposit}
                    className="inline-flex items-center justify-center rounded-lg border border-border bg-white px-3 py-1.5 text-sm font-medium text-foreground/85 shadow-sm hover:bg-muted/40"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <form onSubmit={handleCreateDeposit} className="space-y-6 px-5 py-5 sm:px-6" autoComplete="off">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  <div className="sm:col-span-1">
                    <label htmlFor="deposit-bank" className="mb-1 block text-sm font-medium text-foreground/85">
                      Deposit to (bank register) <span className="text-destructive">*</span>
                    </label>
                    <select
                      id="deposit-bank"
                      ...
                      className="relative z-10 w-full rounded-lg border border-border px-3 py-2 text-sm shadow-sm focus:border-ring focus:outline-none focus:ring-2 focus:ring-ring/20"
                      required
                    >
                ...
                <div>
                  <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <h3 className="text-sm font-semibold text-foreground">Include in this deposit</h3>
                    <button
                      type="button"
                    ...
                    <div className="overflow-x-auto">
                      <table className="min-w-full divide-y divide-border text-sm">
                        ...
                        <tbody className="divide-y divide-border/70 bg-white">
                          {undepositedPayments.map((payment) => {
                            ...
                            return (
                              <tr
                                key={payment.id}
                                className={sel ? 'bg-accent/60' : 'hover:bg-muted/50'}
                              >
                                ...
                                <td className="max-w-[10rem] truncate px-3 py-2.5 text-foreground/85" title={payment.customer_name}>
                                ...
                                <td className="max-w-[8rem] truncate px-3 py-2.5 text-muted-foreground" title={payment.reference_number ?? ''}>
                        ...
                        <tfoot className="bg-muted/40">
                          <tr>
                            <td colSpan={6} className="px-3 py-3 text-right text-sm font-semibold text-foreground/85">
                              Selected total
                            </td>
                ...
                <div className="flex flex-col-reverse gap-3 border-t border-border/70 pt-5 sm:flex-row sm:justify-end">
                  <button
                    type="button"
                    onClick={handleCancelDeposit}
                    className="inline-flex justify-center rounded-lg border border-border bg-white px-4 py-2.5 text-sm font-semibold text-foreground/85 shadow-sm hover:bg-muted/40"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-primary/90 disabled:pointer-events-none disabled:opacity-50"
                  >
                    {submitting ? (
                      <>
          ...
          {!showDepositForm && undepositedPayments.length > 0 && (
            <div className="mb-8 rounded-xl border border-border bg-white shadow-sm">
              <div className="border-b border-border/70 px-5 py-4 sm:px-6">
                <h2 className="text-lg font-semibold text-foreground">Queue — undeposited receipts</h2>
                  ...
                  <tbody className="divide-y divide-border/70 bg-white">
                    {undepositedPayments.map((payment) => (
                      <tr key={payment.id} className="hover:bg-muted/50">
                        ...
                        <td className="max-w-[12rem] truncate px-3 py-2.5 text-foreground/85" title={payment.customer_name}>
                          {payment.customer_name || '—'}
                        </td>
                        <td className="whitespace-nowrap px-3 py-2.5 text-muted-foreground">
                          {formatDateOnly(payment.payment_date)}
                        ...
                        <td className="whitespace-nowrap px-5 py-2.5 text-right font-medium tabular-nums text-foreground">
                          {formatMoney(currencySymbol, payment.amount)}
                        </td>
                      </tr>
                    ))}
          ...
          {!showDepositForm && undepositedPayments.length === 0 && !listError && (
            <div className="mb-8 rounded-xl border border-dashed border-border bg-white px-6 py-12 text-center shadow-sm">
              <Banknote className="mx-auto h-10 w-10 text-muted-foreground/40" aria-hidden />
              <p className="mt-3 font-medium text-foreground">Nothing to deposit</p>
              <p className="mx-auto mt-1 max-w-md text-sm text-muted-foreground">
                Undeposited receipts appear here when you record payments received without selecting a bank
                register. You can also use <Link href="/payments/received" className="font-medium text-primary hover:underline">Payments received</Link> to enter new receipts.
              </p>
            </div>
          )}
          ...
          <div className="rounded-xl border border-border bg-white shadow-sm">
            <div className="border-b border-border/70 px-5 py-4 sm:px-6">
              <h2 className="text-lg font-semibold text-foreground">Recent bank deposits</h2>
              <p className="mt-0.5 text-sm text-muted-foreground">
            ...
            {deposits.length > 0 ? (
              ...
              <p className="px-5 py-10 text-center text-sm text-muted-foreground">No deposits recorded yet.</p>
            )}
          </div>
      </ErpPageShell>
    </PageLayout>
