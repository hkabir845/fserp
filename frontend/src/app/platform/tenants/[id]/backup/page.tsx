    ...
    <PlatformLayout>
      <div className="mx-auto max-w-3xl px-4 py-8">
        <div className="mb-6">
          ...
          <h1 className="mt-2 text-2xl font-bold text-foreground">Tenant backup & restore</h1>
          {tenantQ.data && (
            <p className="mt-1 text-sm text-muted-foreground">
              <span className="font-semibold text-foreground">{tenantQ.data.name}</span>
              <span className="mx-2 text-muted-foreground/70">·</span>
              <span className="font-mono">{tenantQ.data.domain}</span>
              <span className="mx-2 text-muted-foreground/70">·</span>ID {tenantId}
            </p>
          )}
        </div>
        ...
        {getPlatformUser() && (
          <p className="mb-6 text-xs text-muted-foreground">
            Platform operator: <span className="font-mono text-foreground/85">{getPlatformUser()?.email}</span>
          </p>
        )}
        ...
        <section className="mb-8 rounded-lg border border-border bg-white p-6 shadow-sm">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Download a complete logical JSON snapshot (all tables scoped by <code className="rounded bg-muted px-1">tenant_id</code>, users,
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button
              ...
              className="rounded-md border border-border bg-white px-4 py-2 text-sm font-medium text-foreground shadow-sm hover:bg-muted/40 disabled:opacity-50"
            >
              {serverBackupMut.isPending ? 'Saving…' : 'Save backup on server'}
            </button>
          </div>
          {(errExport || errServer) && <p className="mt-2 text-sm text-destructive">{errExport || errServer}</p>}
          {serverBackupMut.data && (
            <div className="mt-4 rounded-md bg-green-50 p-3 text-sm text-green-900">
              ...
              <p className="text-xs text-success">{serverBackupMut.data.size_bytes} bytes</p>
        ...
        <section className="mb-8 rounded-lg border border-border bg-white p-6 shadow-sm">
          <h2 className="text-lg font-semibold text-foreground">Server backup files</h2>
          <p className="mt-1 text-sm text-muted-foreground">Stored under configured <code className="rounded bg-muted px-1">TENANT_BACKUP_DIR</code> on the API host.</p>
          <button
            type="button"
          ...
          {backupsQ.isError && <p className="mt-2 text-sm text-destructive">Could not list backups.</p>}
          <ul className="mt-3 divide-y divide-border/70 text-sm">
            {(backupsQ.data || []).length === 0 && !backupsQ.isLoading && (
              <li className="py-2 text-muted-foreground">No files yet. Use “Save backup on server”.</li>
            )}
            {(backupsQ.data || []).map((f) => (
        ...
        <section className="rounded-lg border border-destructive/25 bg-destructive/5/40 p-6 shadow-sm">
          ...
          <div className="mt-4 space-y-3">
            <div>
              <label className="block text-sm font-medium text-foreground">Confirm tenant domain</label>
              <input
                value={confirmDomain}
            ...
            <div>
              <label className="block text-sm font-medium text-foreground">Backup JSON file</label>
              <input
                type="file"
                accept="application/json,.json"
                className="mt-1 block w-full text-sm text-foreground/85"
          ...
          {errRestore && <p className="mt-2 text-sm text-destructive">{errRestore}</p>}
          {restoreMut.data && (
            <div className="mt-4 rounded-md bg-white p-3 text-sm text-foreground ring-1 ring-red-200">
              <p className="font-medium">{restoreMut.data.message}</p>
              <p className="mt-1 text-xs text-muted-foreground">Tables written: {restoreMut.data.tables_touched}</p>
            </div>
          )}
        </section>
      </div>
