    ...
    <div className="p-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-600">Record customer payments and post them to accounting (Cash/Bank → Accounts Receivable).</p>
        </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-4">
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <div className="text-xs font-semibold text-gray-500">Receipts</div>
          <div className="mt-2 text-2xl font-bold text-gray-900">{stats.count}</div>
        </div>
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <div className="text-xs font-semibold text-gray-500">Total Collected</div>
          <div className="mt-2 text-2xl font-bold text-gray-900">{fmtMoney(stats.total)}</div>
        </div>
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <div className="text-xs font-semibold text-gray-500">Cash</div>
          <div className="mt-2 text-2xl font-bold text-gray-900">{fmtMoney(stats.cash)}</div>
        </div>
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          <div className="text-xs font-semibold text-gray-500">Bank</div>
          <div className="mt-2 text-2xl font-bold text-gray-900">{fmtMoney(stats.bank)}</div>
        </div>
      </div>
      ...
      <div className="mt-4 overflow-hidden rounded-lg border border-gray-200 bg-white">
        {loading ? (
          <div className="p-6 text-sm text-gray-600">Loading receipts…</div>
        ) : error ? (
          <div className="p-6">
            <div className="text-sm font-semibold text-red-700">Could not load receipts</div>
            <div className="mt-1 text-sm text-red-600">{error}</div>
          </div>
        ) : rows.length === 0 ? (
          <div className="p-6">
            <div className="text-sm font-semibold text-gray-900">No receipts found</div>
            <div className="mt-1 text-sm text-gray-600">Create a receipt or switch tenant mode/domain.</div>
          </div>
        ) : (
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-gray-500">Receipt</th>
            ...
            <tbody className="divide-y divide-gray-200 bg-white">
              {rows.map((r) => (
                <tr key={r.id} className="hover:bg-gray-50">
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{r.receipt_number}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{fmtDate(r.receipt_date)}</td>
                  <td className="px-6 py-4 text-sm text-gray-700">{r.customer_name || `Customer #${r.customer_id}`}</td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-semibold text-gray-900">{fmtMoney(Number(r.amount) || 0)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{r.ref_invoice_number || '—'}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4">
          <div className="w-full max-w-xl rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b px-5 py-4">
              <div>
                <div className="text-lg font-bold text-gray-900">New Receipt</div>
                <div className="text-sm text-gray-600">Creates a receipt and posts Cash/Bank → AR.</div>
              </div>
            ...
            <div className="px-5 py-4 space-y-4">
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                ...
                <div>
                  <label className="text-xs font-semibold text-gray-600">Receipt date</label>
                  <input
                    type="date"
              ...
              <div>
                <label className="text-xs font-semibold text-gray-600">Apply to invoice (optional)</label>
                <select
                ...
                <div className="mt-1 text-xs text-gray-500">This is a reference link only (does not yet auto-reconcile invoice balances).</div>
              </div>
