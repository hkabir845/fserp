  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-foreground">Sales requisitions</h1>
        ...
        <table className="min-w-full divide-y divide-border text-sm">
          <thead className="bg-muted/40">
            <tr>
              ...
              <th className="px-4 py-3 text-left font-semibold text-foreground/85">Customer</th>
              <th className="px-4 py-3 text-left font-semibold text-foreground/85">Status</th>
              <th className="px-4 py-3 text-left font-semibold text-foreground/85">Invoice</th>
            ...
            {rows.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-10 text-center text-muted-foreground">
                  ...
                  <Link href="/sales/requisitions/new" className="font-medium text-primary hover:text-primary">
                    Create one
                  </Link>
                  .
                </td>
              ...
              rows.map((r) => (
                <tr key={r.id} className="hover:bg-muted/40/80">
                  ...
                  <td className="px-4 py-3 text-muted-foreground">#{r.customer_id}</td>
                  <td className="px-4 py-3">
                    <StatusPill status={r.status} />
                  </td>
                  <td className="px-4 py-3 text-muted-foreground">
                    {r.converted_invoice_id ? (
                      <Link href={`/sales/invoices`} className="text-primary hover:underline">
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link href={`/sales/requisitions/${r.id}`} className="font-medium text-primary hover:text-primary">
                      Open
                    </Link>
                  </td>
                </tr>
