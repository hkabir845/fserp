...
export default function PermissionMatrix({
  ...
  if (catalog.length === 0) {
    return <p className="text-xs text-warning-foreground">{rt('permLoading')}</p>
  }
    ...
    <div className={className}>
      <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative min-w-0 flex-1">
          <Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground/70" aria-hidden />
          <input
            type="search"
            value={q}
            onChange={(e) => setQ(e.target.value)}
            placeholder={rt('permSearchPlaceholder')}
            className="w-full rounded-lg border border-border bg-white py-1.5 pl-8 pr-2 text-xs text-foreground placeholder:text-muted-foreground/70 focus:border-primary/30 focus:outline-none focus:ring-1 focus:ring-indigo-400"
        ...
        <div className="flex flex-shrink-0 flex-wrap gap-1.5">
          <button
            ...
            className="rounded border border-border bg-white px-2 py-1 text-[11px] font-medium text-foreground hover:bg-muted/40"
          >
            {rt('permAllowAll')}
          </button>
          <button
            type="button"
            onClick={() => onChange([])}
            className="rounded border border-border bg-white px-2 py-1 text-[11px] font-medium text-muted-foreground hover:bg-muted/40"
          >
            {rt('permDenyAll')}
          </button>
        </div>
          ...
          return (
            <div key={g}>
              <div className="flex flex-wrap items-center justify-between gap-1">
                <p className="text-[11px] font-semibold uppercase text-muted-foreground">{groupLabel}</p>
                ...
                {items.map((c) => (
                  <li key={c.id} className="flex items-start gap-2 text-sm text-foreground">
                    ...
                    >
                      {c.label}
                      {showTechnicalIds ? (
                        <span className="ml-1 font-mono text-[10px] text-muted-foreground/70">({c.id})</span>
                      ) : null}
                    </label>
                  </li>
                ))}
