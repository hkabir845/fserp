...
function appendBackupStaleApiHint(message: string): string {
  ...
  return `${message} Current API: ${getApiBaseUrl()}. Use local Django (NEXT_PUBLIC_API_BASE_URL=https://api.mahasoftcorporation.com) or deploy the latest backend to production.`
}
